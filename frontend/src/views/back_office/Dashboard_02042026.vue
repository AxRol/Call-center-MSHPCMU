  ...
  <div class="dashboard">
    <div class="page-header">
      ...
      <div class="header-actions">
        <a href="/front_office/formulaire_appel" target="_blank" rel="noopener" class="btn-form-appel" v-if="isAgent">
...
<!--       <div class="chart-card small">
      ...
      <div class="chart-card small">
        ...
        <div class="chart-wrapper doughnut-wrap">
          ...
          <div class="doughnut-center">
            <span class="doughnut-total">{{ appelsForStats.length }}</span>
            <span class="doughnut-label">Appels</span>
          </div>
        </div>
        <div class="doughnut-legend">
      ...
      <div class="chart-card small">
        ...
        <div class="chart-wrapper doughnut-wrap">
          ...
          <div class="doughnut-center">
            <span class="doughnut-total">{{ ticketsForStats.length }}</span>
    ...
    <div class="table-card">
      ...
      <table class="data-table">
        ...
        <tbody>
          <tr v-if="dashboardTickets.length === 0">
            <td colspan="5" style="text-align:center;padding:30px;color:var(--text-muted)">
              <!-- Message adapté au cas inspecteur -->
              {{ isInspecteur ? 'Aucun ticket ne vous est assigné' : 'Aucun ticket récent' }}
            </td>
          </tr>
          <!--
          ...
          -->
          <tr
            v-for="t in dashboardTickets.slice(0,5)"
            :key="t.id"
            class="clickable-row"
...
const apiAppel  = 'http://localhost:5170/api/appel'
...
// Tickets à afficher:
// - agent      → uniquement les tickets qu'il a créés (créé_par)
// - inspecteur → uniquement les tickets qui lui sont assignés (assigne_a)
// - autres     → tous les tickets
...
const kpiCards = computed(() => [
  {
    id: 1,
    label: 'Appels totaux',
    value: appelsForStats.value.length,
    trend: 12,
  ...
  },
  {
    ...
    value: ticketsForStats.value.filter(t => (t.statut || '').toLowerCase().includes('attente')).length,
    trend: -3,
  ...
  },
  {
    ...
    value: ticketsForStats.value.filter(t => (t.statut || '').toLowerCase() === 'ouvert').length,
    trend: 5,
  ...
  },
  {
    id: 3,
    label: 'Tickets clos',
    value: ticketsForStats.value.filter(t => (t.statut || '').toLowerCase() === 'clos').length,
    trend: 8,
...
const doughnutData = computed(() => {
  const satisfait    = appelsForStats.value.filter(a => a.satisfaction === 'satisfait').length
  const nonSatisfait = appelsForStats.value.filter(a => a.satisfaction === 'non_satisfait').length
  const vide         = appelsForStats.value.filter(a => !a.satisfaction).length
