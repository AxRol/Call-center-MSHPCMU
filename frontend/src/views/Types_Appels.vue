<template>
  <div class="appels-page">
    <div class="page-header">
      <div>
        <h1>liste des types d'appels</h1>
        <p>{{ filteredAppels.length }} types trouvés</p>
      </div>
    ...
    <div class="table-card">
      <table class="data-table">
        ...
        <tbody>
          ...
          <tr v-for="type in paginatedAppels" :key="type.code" :class="{ selected: selectedIds.includes(type.code) }">
           <!--  <td>
              ...
              <div class="modal-body">
                <div class="form-grid">
                  <div class="form-group full">
                    <label>Code *</label>
                    <input v-model="form.code" type="text" placeholder="ex: TAP2" :disabled="modal.type === 'edit'" />
                  </div>
                  <div class="form-group full">
                    <label>Libellé *</label>
                    <input v-model="form.libelle" type="text" placeholder="ex: Canular" />
                  </div>
                </div>
              </div>
...
const allSelected = computed(() => paginatedAppels.value.length > 0 && paginatedAppels.value.every(a => selectedIds.value.includes(a.code)))
function toggleAll(e) {
  if (e.target.checked) selectedIds.value = [...new Set([...selectedIds.value, ...paginatedAppels.value.map(a => a.code)])]
  else selectedIds.value = selectedIds.value.filter(id => !paginatedAppels.value.map(a => a.code).includes(id))
}
